...
public class RateLimitFilter extends OncePerRequestFilter {  //集成该接口并通过注释将其注入到Spring管理的容器中
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        // 管理员Key不受业务限流影响
        Integer rateLimit = (Integer) request.getAttribute("rateLimit"); //从Http请求中获取rateLimit值，从内部获取
        ...
        String keyValue = (String) request.getAttribute("apiKeyValue");
        if (keyValue == null) {
            keyValue = request.getHeader("X-API-Key");
        }

        RateLimitService.RateLimitResult result = rateLimitService.checkRate(
                keyValue != null ? keyValue : "unknown", limit, properties.getWindowSeconds());
